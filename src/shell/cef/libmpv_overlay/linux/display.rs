...
mod tests {
    ...
    #[test]
    fn fractional_and_rotated_outputs_use_physical_monitor_density() {
        let output = Output {
            physical: (3840, 2160),
            logical: (3072, 1728),
            ..Output::default()
        };
        assert_eq!(output.density(), Some(1.25));
        let rotated = Output {
            logical: (1728, 3072),
            ..output
        };
        assert_eq!(rotated.density(), Some(1.25));
        assert_eq!(Output::default().density(), None);
    }

    #[test]
    fn monitor_selection_uses_logical_positions_including_negative_origins() {
        let outputs = [
            Output {
                position: (-3072, 0),
                logical: (3072, 1728),
        ...
        assert_eq!(density_at(&outputs, -100, 800), Some(1.25));
        assert_eq!(density_at(&outputs, 100, 800), Some(2.0));
        assert_eq!(density_at(&outputs, 9000, 800), None);
    }
}
