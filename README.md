...
- **Release calendar.** Agenda and month views of upcoming episodes and film releases. It works from Jellyfin metadata alone and gains monitored/file truth from the optional Companion plugin. The Home **Release Timeline** shelf shows the same releases as posters around a marked today; with the Companion it also leads with the past 30 days' releases, each marked downloaded or missing.
...
Jellyfin 12.1.0 and exposes only typed, authenticated operations. There is no
