...
pkg-config --exists libass lcms2 dav1d gnutls zlib alsa libpulse egl gl libva libva-x11 x11 xext xpresent xrandr xscrnsaver
...
git -C "$sources/libplacebo" submodule update --init --depth=1 3rdparty/fast_float
...
# MediaFlick uses mpv's OpenGL render API, not libplacebo's gpu-next backend.
# Keep libplacebo's required color/shader helpers without another GPU stack.
...
mkdir -p "$output_dir/licenses/fast_float"
cp "$sources/libplacebo/3rdparty/fast_float"/LICENSE* "$output_dir/licenses/fast_float/"
{
...
cp "$script_dir"/*.sh "$script_dir"/*.py "$script_dir"/*.md "$work_dir/recipe/"
...
python3 "$script_dir/smoke-test.py" "$output_dir/lib/libmpv.so.2"
