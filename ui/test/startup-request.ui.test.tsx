...
describe("startup request", () => {
  test("seeds every answered first-frame query from one request", async () => {
    ...
    expect(queryClient.getQueryData(["browsing", account])).toEqual({ last: "/library" })
    ...
    expect(queryClient.getQueryState(["viewing", account])).toBeUndefined()
  })
  ...
  test("asks for Home only when the launch opens on it", async () => {
    ...
    await primeStartupQueries("/library")
  ...
  test("leaves every query to itself when the request fails", async () => {
    ...
    await expect(primeStartupQueries("/")).resolves.toBeUndefined()
