...
- Collections now list from Jellyfin's own BoxSets whenever the Companion plugin advertises native collections: Desktop reads them through ordinary `/Items` queries, and a collection page renders the BoxSet's movies as standard local cards with watched state and server-side ordering, and — where the set carries a TMDB identity — the Seerr-known missing parts underneath with the request flow. BoxSets without a TMDB identity show only their children. When mirroring is off or the plugin predates the capability, Desktop falls back to the derived TMDB summary as before.
