...
- Added TMDB movie collections as a library category. The Companion plugin derives the collection set from the movies the Jellyfin library already contains, resolving each movie's TMDB collection through Seerr with a bounded, concurrent, disk-persisted mapping cache that survives server restarts. The Collections page lists every collection in the library sorted by name ignoring a leading "The", "A", or "An"; a collection page splits its parts in release order into an "In your library" section followed by a "Missing from your library" section, so unreleased parts never lead the page, and missing titles carry the usual discover-and-request flow. Movie details link back to their collection when one exists. The category appears only when the plugin advertises `collections-v1`.
- Added native Jellyfin collections. The Companion plugin can mirror the library's TMDB movie collections into Jellyfin's own BoxSets through a new opt-in setting, so every Jellyfin client sees them. A scheduled task resolves the collection set in bounded background batches, creates each BoxSet with its TMDB provider id, adopts any existing BoxSet with the same id instead of duplicating it, keeps membership in step with the library, and propagates upstream TMDB renames to sets it created or adopted. Turning the setting off stops syncing; collections already created stay in the server. The plugin advertises `collections-v2` while mirroring is enabled.
- Added administrator-defined curated collections. The plugin's dashboard accepts named lists of inline TMDB movie ids or MDBList sources (`username/listname`, `user/username/listname`, or `official/slug`) and mirrors their available movies and series into Jellyfin BoxSets marked with stable MediaFlick provider ids. MediaFlick loads each definition separately from its native BoxSet children, joins movie and series identities against the local library, and shows missing entries with the usual Seerr request flow without delaying library items. The versioned capability remains stable when definitions change during a Desktop session, source and Seerr failures appear in the missing section without breaking BoxSet browsing, and one removed TMDB title no longer hides the rest of a list. MDBList credentials remain on the server, list resolutions use a bounded in-memory cache, and the dashboard check reports movie and series counts before saving.
...
- Collection pages now offer URL-driven sort (release order, name, rating) and a watched filter, matching the library views' controls. Derived parts carry watch state from the local catalog join, so filtering works without hydrating every card.
