...
- Added the foundation for requesting films and shows through Seer (formerly Jellyseerr): its own REST client, error type, and storage. `POST /api/seer/connect` probes an instance and refuses one whose setup wizard is unfinished — an uninitialized Seer treats a login as its setup wizard and would make whoever logs in the instance owner — or one wired to something other than Jellyfin; `GET /api/seer/status` reports whether a link exists along with the user's typed per-media-type request and auto-approve permissions and their quota. The link is the user's own session, never an instance-wide API key, and it is bound to the Jellyfin account it was made under, so signing out or switching users drops it rather than leaving one user's Seer session serving another. Cookies are captured from the very first unauthenticated probe so a Seer with CSRF protection turned on works from the first write onwards, and only GETs are retried, so a request can never be sent twice. No UI yet.
- Added `--seer-status` for headless verification of the Seer link without starting the browser shell.
